		...
		if (animMode == "wave") {
			...
			foreach (Dictionary<Quaternion, VertexData> dict in meshGenerator.vertDict.verts.Values) {
				...
				for (int i = 0; i < curVerts.Length; ++i) {
						meshGenerator.vertices[curVerts[i].verticesIndex] = meshGenerator.vertices[curVerts[i].verticesIndex] + meshGenerator.normals[curVerts[0].verticesIndex].normalized * (Mathf.Sin(2 * Time.time + count) / 400);
				}
				++count;
			}
