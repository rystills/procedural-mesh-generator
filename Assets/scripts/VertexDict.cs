...
public class VertexDict {
	public const float smoothnessFloatTolerance = .001f;//tolerance applied to all direction comparisons to compensate for floating point imprecision
	public int smoothnessFloatDigits = smoothnessFloatTolerance.ToString().Length-2; //inverse of smoothnessFloatTolerance; subtract 2 for decimal place and f character
	public const float normalAverageMaxDifference = 60; //normals of overlapping vertices will not be averaged if their starting normals are larger than this value
